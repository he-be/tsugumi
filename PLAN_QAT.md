...
| D インストールと受入 | **完了・見送り** | 5 ゲート中 2 つ不合格。`RESULTS_QAT.md` |

### 結論 (2026-08-16): **導入見送りを推奨**

詳細は `RESULTS_QAT.md`。要点だけ:

| ゲート | 判定 | 実測 |
| --- | --- | --- |
| 品質 (寿司俳句) | **不合格** | QAT はモーラ数を 3/3 で誤る。ベースラインは 3/3 正答 |
| 性能 | **不合格** | tg −10.9% (story) 〜 −16.5% (math)、TTFT +3.43s |
| メモリ | 合格 | peak 7.84GB (予想 9.3GB より軽い)、64 スロット通過 |
| 目視 / 検証モード | 合格 | |

§5-2-3 で事前に決めた「−10% 前後なら品質が**明確に良ければ**採用可」の
裏返しになった。俳句課題では**ベースラインのほうが明確に良い**ので、
§7 の中止条件どおり見送りを報告する。

**Phase A/B/C の実装はすべて残す** (§0-1 の判断どおり)。判定が成立したのは
group 32 カーネルと BF16 ルーターが正しく動いたからで、§5-A は 20/20 PASS のまま。

### 実測が PLAN の予想と食い違った点

1. **peak は安全側に外れた。**§2 の**導出** 9.3 GB に対し実測 7.84 GB。
   §2 は resident 増と stride 増を単純加算していたが、peak はキャッシュ
   常駐の実効値で決まる。
2. **性能の下振れ要因は 2 つあり、片方は予想外。**未計装ぶん (routed expert
   GEMV) の +12% は §2 の予想どおり。予想外は decode ヒット率の低下
   (math 97.7% → 96.2%、ミス +66%) で、これは group 32 とは無関係な
   **このチェックポイント固有のルーティングの違い**。
3. **haiku がゲートに使えなかった。**QAT は同プロンプトに 20 tok で即答して
   停止するため、384 tok 前提の tg ゲート (≥ 27.8) を評価できない。
   §5-2-3 は「haiku tg」を唯一のゲート値にしていたが、**生成長がモデルで
   変わりうることを織り込んでいなかった。**
4. **アラビア語混入の観察項目は検証不能だった。**temp 1.0 では両モデルとも
   0 件で、混入は temp 0 のベースラインでのみ起きる。
