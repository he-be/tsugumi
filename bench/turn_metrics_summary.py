...
print(f"{'time':19} {'lvl':>4} {'res':>4} {'borrow':>6} {'prompt':>6} {'cached':>6} {'pf tok/s':>8} {'tok/s':>6} {'io ms':>6} {'net':7} ctx")
for r in rows:
    print(f"{r['recordedAt'][:19]:19} {f(r.get('headroomLevel'), 2):>4} {f(r.get('weightsResidentFraction'), 2):>4} "
          ...
          f"{f(prefill_rate(r), 0):>8} {f(r.get('tokensPerSecond')):>6} {f(r.get('ioMillisecondsPerToken'), 2):>6} "
...
print(f"{'level':11} {'n':>3} {'tok/s':>7} {'pf tok/s':>9} {'residency':>9}")
for key in sorted(groups):
    ...
    print(f"{key:11} {len(g):>3} {tok:>7.1f} {f(statistics.median(pf), 0) if pf else '—':>9} "
          f"{f(statistics.median(res), 2) if res else '—':>9}")
