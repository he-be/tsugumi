...
> 2026-08-16 追記: この表の `prefill 秒` はチャンク上限 128 の頃のもの。
> 既定チャンクが 2048 になった今、**prefill はスロット数に依存しない**
> (16 / 48 / 80 で 1% 以内、`docs/investigations/PREFILL_THROUGHPUT.md` §7-1)。
> スロットは decode のためだけに積む。
