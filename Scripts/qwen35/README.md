...
| `build_mtp_sidecar.py` | 差し替え済み MTP ヘッド 42 本を、ランタイムが mmap できる **sidecar** (`mtp_core.bin` 50 MB + `mtp_experts.bin` 453 MB + 目録) に書き出す。エキスパートの blob は**本体の `packed_experts` とバイト並びが同じ**で、`--gturbo` を渡すと `expertStride` を照合する。`.gturbo` の repack は要らない ([docs/qwen35moe/36 §1](../../docs/qwen35moe/36-MTP-DECODE.md)) |
