...
| 5 | 最大の実装障害 | **prefill チャンクの上限 128 トークン** (`PrefillRuntimeConfig.maxChunkTokens`、**実測**)。画像スパン (最大 280) が必ず分割され、双方向 attention が成立しない (§4-5) |
...
| 「チャンク prefill との相互作用は未確認」 | 上限 128 < スパン 280 で**必ず割れる**。プランナ側で解く | §4-5 (**実測**) |
