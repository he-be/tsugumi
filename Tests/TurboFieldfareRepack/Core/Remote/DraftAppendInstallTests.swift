...
struct DraftAppendInstallTests {
    ...
    @Test func addingTheDrafterMatchesAFreshDraftInstall() async throws {
        ...
        #expect(try read(appended, GTurboFormatV1.draftWeightsPath)
                == (try read(fresh, GTurboFormatV1.draftWeightsPath)))
        ...
        #expect(manifest.files[GTurboFormatV1.draftWeightsPath]?.size
        ...
        #expect(receiptFiles[GTurboFormatV1.draftWeightsPath] != nil)
    ...
    @Test func aDrafterCanBeAddedToAModelThatAlreadyHasATower() async throws {
        ...
        let manifest = try GTurboManifestCodec.decode(try read(outputDir, "manifest.json"))
