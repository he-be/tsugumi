...
struct DraftInstallTests {
    ...
    @Test func installsTheDrafterAlongsideTheTextWeights() async throws {
        ...
        let manifest = try GTurboManifestCodec.decode(manifestData)
        #expect(manifest.flags["mtpDraft"] == true)
        #expect(manifest.versionMinor == GTurboFormatV1.versionMinorDraft)
        ...
        #expect(draft.weightsPath == GTurboFormatV1.draftWeightsPath)
        ...
        let entry = try #require(manifest.files[GTurboFormatV1.draftWeightsPath])
        ...
        let indexEntries = try weights.withUnsafeBytes { raw -> [GTurboResidentIndexEntryV1] in
            let header = try GTurboResidentIndexCodec.decodeHeader(raw)
            return try GTurboResidentIndexCodec.decodeRegion(raw, header: header)
        }
        ...
        for name in ["embed_tokens.weight", "pre_projection.weight",
            ...
            #expect(e.dtype == GTurboFormatV1.DType.u32.rawValue)
        ...
        #expect(norm.dtype == GTurboFormatV1.DType.bf16.rawValue)
    ...
    @Test func textOnlyInstallWritesNoDraftArtifacts() async throws {
        ...
        #expect(root["versionMinor"] as? Int == GTurboFormatV1.versionMinor)
    ...
    @Test func addingTheFlagChangesNothingOnTheTextSide() async throws {
        ...
        let a = try GTurboManifestCodec.decode(try read(plain, "manifest.json"))
        let b = try GTurboManifestCodec.decode(try read(withDraft, "manifest.json"))
