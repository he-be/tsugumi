...
struct ServerLifecycleTests {
    ...
    func LIF_7_a_failed_load_exits_the_process() async throws {
        ...
        process.arguments = [
            "--model", "/nonexistent/tsugumi-tests/no-such-model.moepack",
            "--port", "\(port)",
        ]
        let output = Pipe()
        let errors = Pipe()
    ...
    private static let freshServerBinary: URL? = {
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let sources = [
            "Sources/TsugumiServer/Command/main.swift",
            "Sources/TsugumiServer/Core/HTTPServer.swift",
        ].compactMap {
        ...
        for configuration in ["debug", "release"] {
            let candidate = root.appendingPathComponent(
                ".build/\(configuration)/TsugumiServer")
