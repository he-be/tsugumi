...
struct ServerLifecycleCORSTests {
    ...
    @Test func FLAG_6_the_flag_takes_a_list_or_a_star() throws {
        #expect(try ServerArguments.parse(["--model", "m.moepack"]).corsPolicy
                == .disabled)
        #expect(try ServerArguments.parse(["--model", "m.moepack",
        ...
        #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.moepack", "--cors-origins", " , "])
        }
    }
}
