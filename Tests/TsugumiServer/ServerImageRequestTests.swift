...
struct ServerImageRequestTests {
    ...
    @Test func bodyCeilingCoversTheConfiguredImages() {
        ...
        #expect(ServerImagePolicy(maxImagesPerRequest: 0).maximumBodyBytes
                == TsugumiHTTPServer.maximumBodyBytes)
    }
    ...
    @Test func serverArgumentsCarryTheImagePolicy() throws {
        ...
        #expect(try ServerArguments.parse(["--model", "m.moepack"]).imagePolicy == .default)
        #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.moepack", "--image-tokens", "560"])
        }
        #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.moepack", "--max-images", "0"])
        }
    }
