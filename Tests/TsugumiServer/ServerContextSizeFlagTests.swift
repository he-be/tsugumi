...
struct ServerContextSizeFlagTests {
    ...
    @Test func FLAG_1_context_size_takes_the_reference_spelling() throws {
        #expect(try ServerArguments.parse(["--model", "m.moepack"]).maxContext == 16_384,
                "既定は変わらない")
        for spelling in ["-c", "--ctx-size"] {
            for tokens in [4_096, 8_192, 16_384, 32_768, 65_536, 131_072] {
                let arguments = try ServerArguments.parse([
                    "--model", "m.moepack", spelling, String(tokens),
                ])
        ...
        for value in ["", "16k", "sixteen"] {
            #expect(throws: ServerArgumentError.self) {
                try ServerArguments.parse(["--model", "m.moepack", "--ctx-size", value])
            }
        }
    }
    ...
    @Test func FLAG_1_max_context_is_retired_and_the_error_names_ctx_size() throws {
        for arguments in [["--model", "m.moepack", "--max-context", "16384"],
                          ["--model", "m.moepack", "--max-context", "100000"],
                          ["--model", "m.moepack", "--max-context"]] {
        ...
        #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.moepack", "--max-context", "4096"])
        }
    }
    ...
    @Test func FLAG_2_ctx_size_rounds_down_to_a_supported_size() throws {
        ...
        for (requested, effective) in expected {
            let arguments = try ServerArguments.parse([
                "--model", "m.moepack", "--ctx-size", String(requested),
            ])
        ...
        for value in ["0", "-1", "-4096"] {
            #expect(throws: ServerArgumentError.self) {
                try ServerArguments.parse(["--model", "m.moepack", "-c", value])
            }
        }
    }
