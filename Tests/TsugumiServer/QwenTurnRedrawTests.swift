...
/// `.moepack`, which lives under `scratch/` and is not in the repository, so a
/// checkout that has not installed Ornith has nothing to load here.
private enum OrnithTokenizerSidecar {
    /// Either install directory: one made after the container was renamed to
    /// `.moepack`, or one from before, which the runtime still reads. A
    /// machine that has the checkpoint should run this suite whichever name
    /// its directory carries — silently skipping is the failure mode this
    /// gate is most likely to hide.
    static let folder: URL = {
        let names = ["scratch/ornith-oq4e-g64.moepack", "scratch/ornith-oq4e-g64.gturbo"]
        let candidates = names.map {
            URL(fileURLWithPath: $0).appendingPathComponent("tokenizer")
        }
