...
struct ServerReasoningRequestTests {
    ...
    @Test func FLAG_1_reasoning_format_is_a_process_flag() throws {
        #expect(try ServerArguments.parse(["--model", "m.moepack"]).reasoningFormat == .auto)
        #expect(try ServerArguments.parse(
            ["--model", "m.moepack", "--reasoning-format", "none"]).reasoningFormat == .none)
        #expect(try ServerArguments.parse(
            ["--model", "m.moepack", "--reasoning-format", "auto"]).reasoningFormat == .auto)
        #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.moepack", "--reasoning-format", "deepseek"])
        }
    }
    ...
    @Test func FLAG_4_thinking_is_retired_and_the_error_names_its_replacement() throws {
        for arguments in [["--model", "m.moepack", "--thinking", "on"],
                          ["--model", "m.moepack", "--thinking", "off"],
                          ["--model", "m.moepack", "--thinking"]] {
        ...
        let cacheMode = #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.moepack", "--prompt-cache-mode", "off"])
        }
        #expect(cacheMode?.description.contains("cache_prompt") == true)
        // An unknown flag is still just an unknown flag.
        #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.moepack", "--reasoning", "on"])
        }
...
struct ServerReasoningResponseTests {
    @Test func nonStreamingCarriesReasoningContentBesideTheAnswer() async throws {
        let server = TsugumiHTTPServer(modelID: "test-model",
                                              queueLimit: 1,
