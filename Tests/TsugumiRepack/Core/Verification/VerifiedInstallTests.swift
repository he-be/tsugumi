...
@Suite struct VerifiedInstallTests {
    @Test func acceptsRootSymlinkAndBindsAlias() throws {
        ...
        let alias = temporaryURL("verified-install-link.moepack")
        defer {
        ...
        let result = try VerifiedInstallTool.run(
            options: VerifyInstallOptions(inputMoEPack: alias.path))
    ...
    @Test func concurrentWritersPublishCompleteReceipt() async throws {
            ...
            for _ in 0..<8 {
                group.addTask {
                    _ = try VerifiedInstallTool.run(
                        options: VerifyInstallOptions(inputMoEPack: root.path))
                }
            }
        ...
        #expect(!names.contains { $0.hasPrefix(".moepack-") && $0.hasSuffix(".tmp") })
    }
    ...
    private func makeInstall(manifestBytes: UInt64? = nil,
        ...
        let root = temporaryURL("verified-install.moepack")
        try FileManager.default.createDirectory(
            at: root.appendingPathComponent("packed_experts"),
            withIntermediateDirectories: true)
        let stride = MoEPackFormatV1.alignmentBytes
        let modelData = Data([0])
        let layerData = Data(repeating: 0, count: Int(stride))
        var layoutData = try MoEPackPackedExpertsLayoutCodec.encode(
            MoEPackPackedExpertsLayoutV1(
                ...
                layers: [MoEPackLayerV1(
                    layer: 0,
                    file: "layer_00.bin",
                    experts: [MoEPackExpertV1(
                        expert: 0,
                        physicalRank: nil,
                        offset: 0,
                        size: stride,
                        tensors: ["gate": MoEPackSubTensorV1(
                            offset: 0,
                            size: 32,
                            dtype: "U32",
                            shape: [8, 8],
        ...
        let files = [
            "model_weights.bin": MoEPackManifestFileV1(
                size: UInt64(modelData.count), sha256: hash(modelData)),
            "packed_experts/layout.json": MoEPackManifestFileV1(
        ...
        var manifestData = try MoEPackManifestCodec.encode(MoEPackManifestV1(
            flags: [
    ...
    private func fixtureArch() -> MoEPackManifestArchV1 {
    ...
    private func temporaryURL(_ suffix: String) -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-\(UUID().uuidString)-\(suffix)")
    }
}
