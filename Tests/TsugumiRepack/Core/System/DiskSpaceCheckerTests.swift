...
@Suite struct DiskSpaceCheckerTests {
    @Test func assessmentDoesNotCreateMissingTargetAndUsesExistingAncestor() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("tsugumi-space-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let target = root.appendingPathComponent("nested/model.moepack", isDirectory: true)
    ...
    @Test func authoritativeCheckUsesSameRequirement() throws {
        ...
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let target = root.appendingPathComponent("model.moepack", isDirectory: true)
