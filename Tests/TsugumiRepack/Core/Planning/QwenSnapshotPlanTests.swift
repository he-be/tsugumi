...
struct QwenSnapshotPlanTests {
    ...
    @Test func planSplitsExpertsFromResidentAndLeavesTheDrafterOut() throws {
        ...
        for layer in plan.layers {
            ...
            #expect(layer.expertStride % MoEPackFormatV1.alignmentBytes == 0)
        }
    ...
    @Test func manifestCarriesTheFamilyAndItsFlag() throws {
        ...
        let data = try MoEPackJSON.encodeManifest(
            ...
            files: [("model_weights.bin", MoEPackJSON.FileEntry(size: 16_384, sha256: zeroSHA))],
            expertsPerLayer: arch.numExperts,
            numLayers: arch.numLayers,
            expertStride: stride,
            bitWidths: MoEPackJSON.QuantBitWidths(embedding: 8, attention: 4, router: 16,
                                                 sharedExpert: 8, routedExpert: 4))
        let manifest = try MoEPackManifestCodec.decode(data)
        ...
        #expect(manifest.versionMinor >= MoEPackFormatV1.versionMinorLinearAttention)
