...
struct RepackCLITests {
    ...
    @Test func visionIsNotAModeForVerificationOrDiscard() throws {
        ...
        let verify = try run(["--verify-install", "--input-moepack", output,
                              "--include-vision"])
        #expect(verify.status == 2)
        #expect(verify.stderr.contains("verification accepts only --input-moepack"))
    }
    ...
    @Test func addVisionTakesOnlyAnInstalledModel() throws {
        ...
        #expect(missingInput.stderr.contains("missing required argument: --input-moepack"))

        let withOutput = try run(["--add-vision", "--input-moepack", output,
                                  "--output", output])
        #expect(withOutput.status == 2)
        #expect(withOutput.stderr.contains("--add-vision accepts only --input-moepack"))

        let redundant = try run(["--add-vision", "--input-moepack", output,
                                 "--include-vision"])
    ...
    @Test func addVisionRefusesAPathThatHoldsNoModel() throws {
        ...
        defer { clean(output) }
        let result = try run(["--add-vision", "--input-moepack", output])
        #expect(result.status == 1)
    ...
    private func run(_ arguments: [String]) throws
        ...
        let executable = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent(".build/debug/TsugumiRepack")
        let process = Process()
        let stdout = Pipe()
        let stderr = Pipe()
    ...
    private func temporaryOutput(_ tag: String) -> String {
        (NSTemporaryDirectory() as NSString)
            .appendingPathComponent("tsugumi-cli-\(tag)-\(UUID().uuidString).moepack")
    }
