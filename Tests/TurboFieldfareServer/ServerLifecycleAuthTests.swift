...
struct ServerLifecycleAuthTests {
    ...
    @Test func FLAG_5_the_flag_takes_a_comma_separated_list() throws {
        let single = try ServerArguments.parse(["--model", "m.gturbo",
                                                "--api-key", "one"])
        #expect(single.apiKeys == ["one"])

        let several = try ServerArguments.parse(["--model", "m.gturbo",
                                                 "--api-key", "one,two , three"])
        #expect(several.apiKeys == ["one", "two", "three"])

        let repeated = try ServerArguments.parse(["--model", "m.gturbo",
                                                  "--api-key", "one",
                                                  "--api-key", "two"])
        #expect(repeated.apiKeys == ["one", "two"])

        #expect(try ServerArguments.parse(["--model", "m.gturbo"]).apiKeys == [])

        #expect(throws: ServerArgumentError.self) {
            try ServerArguments.parse(["--model", "m.gturbo", "--api-key", " , "])
        }
    }
}
