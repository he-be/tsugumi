...
@Suite struct Sha256VerifierTests {
    ...
    @Test func hashesEmptyFile() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-sha-empty-\(UUID().uuidString).bin")
    ...
    @Test func chunkSizeDoesNotAffectDigest() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-sha-2m-\(UUID().uuidString).bin")
    ...
    @Test func verifyMatches() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-sha-match-\(UUID().uuidString).bin")
    ...
    @Test func verifyMismatchThrowsChecksumMismatch() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-sha-bad-\(UUID().uuidString).bin")
