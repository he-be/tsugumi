...
@Suite struct ManifestReaderTests {
    ...
    static func writeToyManifest(_ overrides: [String: Any] = [:],
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-manifest-test-\(UUID().uuidString)")
    ...
    @Test func missingManifestThrowsPartialInstall() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-empty-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
        #expect {
    ...
    @Test func wrongMagicThrowsNotAMoEPackDirectory() throws {
        let (dir, toy) = try Self.writeToyManifest(["magic": "NOT_MOEPACK"])
        defer { try? FileManager.default.removeItem(at: dir) }
        #expect(throws: ModelError.notAMoEPackDirectory) {
            _ = try ManifestReader.load(directoryURL: dir, expecting: toy)
        }
    }
