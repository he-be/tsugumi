...
@Suite struct ResidentIndexTests {
    ...
    @Test func roundtripsWriterEncodedBytes() throws {
        ...
        let headerBytes = MoEPackBinary.indexHeaderBytes
        let entryBytes  = MoEPackBinary.indexEntryBytes
        ...
        let indexBytes = Int(MoEPackFormatV1.alignmentBytes)
        ...
        fileBuf.withUnsafeMutableBytes { raw in
            ...
            for (i, e) in entries.enumerated() {
                ...
                MoEPackBinary.writeIndexEntry(into: dst, entry: e,
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-index-roundtrip-\(UUID().uuidString).bin")
    ...
    @Test func shortFileThrowsIndexCorrupt() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-short-\(UUID().uuidString).bin")
        defer { try? FileManager.default.removeItem(at: url) }
    ...
    @Test func acceptsIndexRegionExactlyAtMetadataCap() throws {
        ...
        data.withUnsafeMutableBytes {
            MoEPackBinary.writeIndexHeader(
                into: $0.baseAddress!, indexSize: cap,
                residentSize: 0, entryCount: 0)
        }
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-index-cap-\(UUID().uuidString).bin")
        defer { try? FileManager.default.removeItem(at: url) }
    ...
    @Test func rejectsIndexRegionAboveMetadataCapBeforeAllocation() throws {
        ...
        var data = Data(repeating: 0, count: MoEPackBinary.indexHeaderBytes)
        data.withUnsafeMutableBytes {
            MoEPackBinary.writeIndexHeader(
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-index-over-cap-\(UUID().uuidString).bin")
        defer { try? FileManager.default.removeItem(at: url) }
        try data.write(to: url)
        #expect {
    ...
    @Test func rejectsIndexSizeSmallerThanHeader() throws {
        ...
        data.withUnsafeMutableBytes {
            MoEPackBinary.writeIndexHeader(
                into: $0.baseAddress!, indexSize: UInt64(MoEPackBinary.indexHeaderBytes - 1),
                residentSize: 0, entryCount: 0)
        }
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-index-small-header-\(UUID().uuidString).bin")
        defer { try? FileManager.default.removeItem(at: url) }
        try data.write(to: url)
    ...
    @Test func callerSelectedSymlinkToRegularIndexStillLoads() throws {
        let indexSize = Int(MoEPackFormatV1.alignmentBytes)
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-index-symlink-\(UUID().uuidString)")
    ...
    @Test func symlinkToFIFOIsRejectedWithoutReading() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("moepack-index-fifo-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
