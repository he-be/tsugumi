...
@Suite struct AppModelLoadStateTests {
    ...
    @Test func runRequiresReadyState() {
        ...
        #expect(!model.canRun)
        model.loadState = .ready(modelDirectory: URL(fileURLWithPath: "/tmp/m.moepack"), loadSeconds: 1.2)
        #expect(model.canRun)
    }
    ...
    @Test func staleReadyStateForOldModelPathIsIgnored() {
        ...
        let oldURL = URL(fileURLWithPath: "/tmp/old.moepack")
        let newURL = URL(fileURLWithPath: "/tmp/new.moepack")
