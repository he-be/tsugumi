...
@Suite struct AppChatStoreTests {
    ...
    /// The rename moved the store from Application Support/TurboFieldfare to
    /// .../Tsugumi. Chats written before it must come back, or the rename
    /// reads to the user as data loss.
    @MainActor
    @Test func chatsWrittenUnderTheOldApplicationNameAreStillRead() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("chat-rename-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let legacyURL = root.appendingPathComponent("TurboFieldfare", isDirectory: true)
            .appendingPathComponent("chats.json", isDirectory: false)
        let store = AppChatStore(
            fileURL: root.appendingPathComponent("Tsugumi", isDirectory: true)
                .appendingPathComponent("chats.json", isDirectory: false),
            legacyFileURL: legacyURL)

        var persisted = PersistedChats()
        persisted.chats = [PersistedChat(outputPromptText: "from the old name")]
        try AppChatStore(fileURL: legacyURL).save(persisted)

        let model = readyModel(client: MockInferenceClient(), chatStore: store)

        #expect(model.chats.count == 1)
        #expect(model.chats[0].outputPromptText == "from the old name")
    }
