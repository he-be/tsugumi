...
| --- | --- | --- | --- | --- |
| Expert-cache slots | 8, 16, 24, 32, 48, 64, 80, 96, 112 | `--expert-cache-slots` | 48 | More slots retain more routed experts and reduce later reads, at about 100 MB each. Expert reads overlap GPU work, so past the point where they stop poking out from behind it, extra slots buy hit rate and no throughput. 48 is that point for the pinned QAT checkpoint; 32 is measurably slower. The group-64 lineage peaks at 64 instead, so pass `--expert-cache-slots 64` when running one. Chunked prefill requires at least 16 slots. |
...
| Reasoning | — | `--thinking on\|off` | Off | Opens the chat template's thought channel, so the model reasons before it answers. The reasoning is generated text: it spends the same token budget as the answer, and it comes back separately (server: `reasoning_content`) rather than mixed into it. The CLI flag applies to `--messages-file` only. On the server it is the process default, which a request overrides with `chat_template_kwargs.enable_thinking` or `reasoning_effort`; a request that declares tools reasons in neither case, because the tool-calling template pins the channel closed. |
