...
> **2026-08-22 に置き換わった ([42](42-SAMPLING.md))。**以下の「受理して無視」は
> **もう本線ではない**。公式推奨サンプラ (0.6 / 0.95 / 20) を実際に使うのが要件
> S1 で、サーバーは要求が何を送っても**その値で上書きして実行**し、完了行に
> `sampling/official-override: …` と記録する。融合ヘッドは logit を書く版
> (`qwen_lm_head_logits_int8_rows_chunk_multi`) を持ち、MTP は投機サンプリングで
> 走り続ける (S2)。**番号の大きい 42 が正。**以下は当時の記録として残す。
