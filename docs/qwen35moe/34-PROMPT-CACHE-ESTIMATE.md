...
> **2026-08-22 に実装して測った: [41](41-PROMPT-CACHE.md) が正。**取り分の
> 見積もり (§1-5 の 9.16 秒) は当たり、床 1.30 秒は**保守的すぎた** (実測の
> 継続は 0.72 秒)。「一番大きい危険」と呼んだ再レンダの継ぎ目 (§4-3) は
> **実測ではずれなかった**。式で 1 つ外れていたのは §3-1 の
> 「KV-backed = promptIDs + produced.dropLast()」で、**幅 2 の投機が受理して
> 終わったパスは最後のトークンも食っている**。§3-3 が挙げた穴 3 つは全部実在し、
> **4 つ目 (MTP ヘッドのキャッシュ) があった**。
