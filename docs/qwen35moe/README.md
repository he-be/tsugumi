...
| 29 | [33-MTP-ACCEPTANCE.md](33-MTP-ACCEPTANCE.md) | **実測(手元) + 導出。**Phase 7 の M0 — 受理率 P1 と受理長 a をタスク別に、実トレースから引いた検証幅ごとのエキスパート和集合とその 2 度の訂正 (§3-4 / §3-5)、**幅 2 の 1 パス費用の実測 (1.27〜1.30 倍) と取り分 +15〜29%** (§3-7)、**GDN の snapshot/restore はコピー 0 回にできる** (§3-6)、参照器とランタイムの 192 トークン一致率 |
