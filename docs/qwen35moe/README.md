...
| 38 | [43-LILY-IDEAS.md](43-LILY-IDEAS.md) | **検討 (公表値のみ)。**Perplexity の Apple silicon 専用 Qwen3.6-35B-A3B エンジン Lily (MLX-LM 比 prefill 1.23 / decode 1.35 倍、M5 Max 128 GB 常駐) の 14 項目と本線の対応 — **8 つは済**、写せる未実装は **GQA packing** (M6 に依らず今) と **routed MoE prefill GEMM の `matmul2d` 化** (M6 待ち、prefill は io 側が重いので条件付き)、記事の「投機デコードは 18% 遅い」は常駐前提で本線には当てはまらない、写さないもの 4 つ (GPU 完結 routing・GPU 内トークン受け渡し・concurrent dispatch・32K 超のレイアウト切替)。機種側は [investigations/M6_MAC_MINI_TARGETING.md](../investigations/M6_MAC_MINI_TARGETING.md) |
...
- Lily: [Optimizing On-Device Inference for Apple Silicon](https://www.perplexity.ai/hub/blog/optimizing-on-device-inference-for-apple-silicon) / [pplx-garden/lily](https://github.com/perplexityai/pplx-garden/tree/main/lily) — Perplexity の Apple silicon 専用 Qwen3.6-35B-A3B エンジン。対応表と写し先は [43](43-LILY-IDEAS.md) (ソース未読・公表値のみ)
