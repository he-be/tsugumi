...
| 4 | ~~**tensor ops を使わないまま移植したときの pp**~~ **済 (2026-09-22): pp 288（GPU 側 7.63 s）** | 現行コードを macOS 27 の CLT でビルド、2478 トークン | §2-2-1。**遅くならない。12 コアで M3 Pro 18 コアの 1.22 倍**（§7-4a）。**比の分母が確定した** |
| 5 | ~~macOS 27 で deployment target の引き下げ（`103bfbc`）を戻せるか~~ **済 (2026-09-22): 戻せる。ただし戻すべきではない** | `.macOS(.v26)` に上げて CLT でビルド（74.9 s、エラー 0、minos 26.0） | 上流想定に戻れる = 上流ベンチと直接比較できる。**が、MBP が動かなくなる**（§7-5a） |
...
### 7-4a. #4 の実測 — 12 コアでも遅くならない。GPU 側は 1.21 倍（2026-09-22、**実測**）

`gemma4-qat-sym.gturbo`（attention は group 32 / sym なので **MPP 経路は閉じている** = §7-1a。
まさに「tensor ops を使わないまま」の条件）、2478 トークン、`--expert-cache-slots 32
--prefill-chunk-tokens 2048 --max-new 1 --max-context 4096 --verification full-sha256`、
両機とも同じ commit（`c5a072d`）・同じプロンプト。各 6 本。

| | prefill 壁時計（中央値） | **pp** | io | **GPU 側（prefill − io）** | layerVerify |
| --- | ---: | ---: | ---: | ---: | ---: |
| **M6**（12 コア、macOS 27、Swift 6.4） | **8.60 s**（8.40〜8.75 + 外れ値 1 本） | **288 tok/s** | 0.77〜1.08 s | **7.63 s**（7.56〜7.69） | 4.3〜4.5 s |
| **MBP**（M3 Pro 18 コア、macOS 15） | 10.53 s（10.47〜10.62） | 235 tok/s | 1.23〜1.33 s | 9.26 s（9.19〜9.38） | 5.5〜5.7 s |
| 比 | 1.22 | **1.22** | — | **1.21** | 1.27 |

**結論: §2-2-1 が心配した「12 コアで遅くなる」は起きない。** tensor ops を一切使わずに、
GPU 側で 1.21 倍速い。**この 7.63 s が、M6-2 以降の「N 倍」を語るときの分母である。**

分かったことが 2 つある:

1. **io は GPU と重なっていない。**io が 0.85 s のときも 3.33 s のときも GPU 側は 7.62〜7.69 s で動かず、
   壁時計が io の差のぶんだけ丸ごと伸びる。prefill のチャンク境界で読みと計算が直列になっている。
   mflux が M8b で `async_eval` を使って隠した形（見えている I/O 137 → 1 ms）が、そのまま効く余地である。
   **M6 は SSD が半分なので、この直列は M3 Pro より高くつく。**
2. **ページキャッシュの効き方が両機で違う。**MBP の io は 1.23〜1.33 s で安定しているのに対し、
   M6 は 0.77〜1.08 s に混じって **3.33 s が 1 本**出た（rsync 直後の 1 回目）。
   21 GB を外付けから読んでキャッシュを流す手では再現できず、**原因は未確認**。
   `sudo purge` が使えないので冷温の強制ができていない。**16 GB 機では io の条件を毎回記録すること。**

> 比較の注意: §2-1 の基準値 pp 231 は **48 スロット**・別ビルドの値である。
> 現行 CLI は `--expert-cache-slots` が 8/16/24/32 までなので 48 は再現できない。
> 上の表は **両機を今日・同じビルド・32 スロットで取り直した対**で、MBP の 235 が 231 とよく一致するので、
> 基準値との接続は取れている。

### 7-5a. #5 の実測 — 戻せるが、戻す理由が無い（2026-09-22、**実測**）

`Package.swift` の `.macOS(.v15)` を `.v26` に上げ、M6 の CLT（Swift 6.4 / SDK 27.0）で
`swift build -c release --product TsugumiCLI` → **エラー 0、74.9 秒で通る**。
バイナリの `minos` は 26.0 になり、戻してビルドし直すと 15.0 に戻る（`bench/hostinfo.sh` で確認）。

**それでも上げない。**理由:

1. **tensor 経路の挙動は変わらない。**MSL 4.0 の選択は deployment target ではなく
   実行時の `#available(macOS 26.0)`（`MetalContext.shaderLanguageVersion`）で決まる。
   §7-1a のとおり M6 で経路が閉じているのはモデルが group 32 だからで、target とは無関係である。
2. **MBP が動かなくなる。**macOS 15.7.5 では minos 26.0 のバイナリを dyld が拒否する（`103bfbc` の本文）。
   [2 機体制](../TWO_MACHINE_DEV.md) は「MBP で macOS 15 の回帰を見る」が前提で、これが崩れる。
   [CONTRIBUTING](../../CONTRIBUTING.md) の「macOS 15 and newer」とも矛盾する。
3. 上げて得られるのは「上流ベンチと直接比較できる」だけで、**それは上流を M6 で回せば済む**。

結論: **#5 は「できることを確認した」で閉じる。**上流想定に戻すのは、MBP を macOS 26 以降に
上げるか、開発機から外すと決めたときでよい。
