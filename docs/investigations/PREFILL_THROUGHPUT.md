...
作成: 2026-08-16
更新: 2026-08-16 (射影と共有 MLP を simdgroup_matrix int4 GEMM に、§7-8)
目標: **300 pp tok/s**。表記は PLAN.md に合わせる: **実測** / **導出** / **未確認**

**現在地: 2478 トークンで 160.6 pp。調査開始時 (1973 トークン) は 37.2、
同じ 2478 トークンでの出発点は 70.2。**
1973 トークンでの 78.5 は §7-1 時点の値。
何をしたかは §7 (§7-8 が最新)。以下 §1-§6 は調査時点の記録で、
...
- **次の的は routed MoE の 8.59 s ただ一つ** (GPU 13.44 s の 64%、§6 Gate 3)。
  射影・共有 MLP で使ったタイル構成 (64×64 出力、重みを threadgroup に dequant、
  8×8 `simdgroup_matrix`) がそのまま expert 単位 GEMM の雛形になる
