...
| 通常 | 結果がまだ無いラウンドだけ思考予算 (既定 512、RSN-4 の閉じタグ強制)。結果を読んでからのラウンドは無制限 | 上限が立つ。2026-09-04 までは締切を持つラウンドが**最初から** plain 経路に落ちていて (DEV-14)、「代償は呼び出し 30 トークン程度」という見積もりは外れていた — 思考を含めて 450〜540 トークンが 22 tok/s (投機ありの半分) だった (turn-metrics 実測)。forcer を投機ループに通して解消 |
