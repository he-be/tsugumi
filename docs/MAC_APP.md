...
思考チャネルのライブ表示は**追記だけする NSTextView** (`LiveReasoningTextView`、
差分は `ReasoningLivePresentation.edit`)。SwiftUI の `Text` はトークン毎に文字列全体を
CoreText で計測・描画し直す。2026-09-01 に全文を末尾 1,500 字に切ったが、それでも
Qwen3.8 の思考 150 秒でメインスレッドの 76% を使い (計測 38%・描画 37%)、Stop の
クリックが 22 秒後 (生成の終了後) に処理された (2026-09-15、sample と GUI 操作で再現)。
追記にしてからは同じ場面で待機 88%、Stop はクリックから 1.4 秒で decode service が
止まる。完了後のディスクロージャも同じビューで全文を出す。
アプリが終了して decode service の入力が切れたときも生成を取り消す (以前は最後まで
decode していた)。
