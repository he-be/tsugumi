...
5. ~~**思考 ON の要求はプロンプトキャッシュに参加しない**~~ →
   **2026-08-19 に撤回。参加する** (§5)。切っていた理由が誤りで、
   pi の対話が毎ターン全再 prefill になっていた。
...
## 5. プロンプトキャッシュ — 一度切って、戻した (2026-08-19)

**最初の判断 (誤り):** `--prompt-cache-mode single-prefix` は直前の生成の KV を
次の要求の前置きとして使い回す。思考 ON だとその KV には思考トークンが入るが、
次のターンを素で描き直すと思考は入らない。「ヒットとミスで別のプロンプトになる
のは再現性を壊す」と考えて、読み書きの両側で外した。

**実際に起きたこと:** pi の対話セッションは毎要求 `enable_thinking: true` を
送るので、**全ターンが全再 prefill になった**。3,700〜4,100 トークンの
プロンプトで 1 ターン 18〜19 秒 — そのほとんどが prefill で、生成は 60〜90
トークンしかない、という状態である。

**撤回の理由:** 「キャッシュ経由の履歴は素の描き直しと一致する」という前提が、
**思考 OFF でもそもそも成り立っていなかった**。キャッシュが持ち回る履歴は
`<|turn>model\n<|channel>thought\n<channel|>` + 答え + `<turn|>` で、
素の描き直し (`<|turn>model\n` + 答え + `<turn|>`) とは空の thought channel の
ぶんだけ違う。このキャッシュの設計はもともと **KV が履歴である**という立場で、
思考にだけ厳しい基準を当てていたことになる。

**今の挙動:** 思考 ON でも参加する。ただし**モードはキャッシュのキーに入れた**
(`ServerPromptCacheEntry.enableThinking`) — 思考 ON の KV は system ターンに
`<|think|>` を持ち、継続ブリッジの末尾も違う (思考 ON は thought channel を
開いたまま、OFF は開いて閉じる) ので、**モードが違えばミスになる**。

再利用の実測 (system 約 1,000 トークン + tools 宣言 + 3 ターン追記):

| | turn 1 | turn 2 | turn 3 |
| --- | ---: | ---: | ---: |
| 思考 ON: prompt / cached | 1,036 / 0 | 1,080 / **1,062** | 1,104 / **1,086** |
| 思考 ON: 壁時計 | 6.41 s | **0.91 s** | **0.91 s** |
| 思考 OFF: prompt / cached | 1,038 / 0 | 1,061 / 1,039 | 1,085 / 1,063 |
| 思考 OFF: 壁時計 | 4.27 s | 1.00 s | 0.79 s |
