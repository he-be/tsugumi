...
モデルの重みも Metal も要らない。既知の破れ (2026-08-19 実測、
`PromptTokenInvariantTests`): 思考 OFF は空の thought channel の 12 トークン分、
思考 ON はそれに加えて `reasoning_content` が入力として受け取られないぶん
思考ブロックが丸ごと落ちて 20 トークン分 + 思考の長さ。どちらも
「直前の assistant ターンを毎回描き直す」ぶんだけ LCP が短くなる。
