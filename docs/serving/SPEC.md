...
ので、`Tests/` を ID で引けば、その行がどこで守られているかが出る。
この文書は実験ログではなく**生きた文書**
...
| EP-3 | `POST /v1/chat/completions` | §4〜§9 | 実装済 (乖離あり) |
...
| GEN-4 | `tool_choice` は 4 形すべてをサンプラの拘束で実現する: `none` は文法を持たず、**ツール呼び出しの開始トークン** (Gemma `<\|tool_call>`、Qwen `<tool_call>`) の id だけを候補から落とす (`ForbiddenTokensConstraint`、それ以外と終了は常に可、宣言は残す。プロンプトの文で呼び出しを抑えることはしない、`docs/qwen38/26` §2)、`auto` は**遅延文法** (GEN-5)、`required` と名前指定は**最初から拘束する** (非遅延)。名前指定は文法が関数名そのものを固定する (DEV-17)。**文法では作れない拒否は要求の側で 400 にする**: 名前指定が**宣言されていないツール**を指した場合と、`required` なのに `tools` が空の場合 — どちらも `invalid_request_error`。文法を空にして黙って自由生成に落とすことは R4 違反。 |
| GEN-8 | **tool call の文法は、テンプレートが描き直す正準形だけを許す。**すなわち: 空白を一切入れない、オブジェクトのキーは**裸**で**昇順** (テンプレートの `dictsort`。Qwen の引数の順は DEV-15)、文字列は `<\|"\|>…<\|"\|>` のみ (JSON の `"…"` は**生成では許さない** — 読む側 `GemmaToolCallParser` は互換のため両方受ける)。理由は §7 **INV-1**: 完了した tool call ターンは引数を parse 済みの値から描き直すので、生成が正準形でなければ描き直しと必ずずれ、毎ターン LCP が切れる。数値の正準化 (`1.50` と `1.5`) までは踏み込まない — ずれても縮むのは LCP だけで、応答は変わらない。 **両端のマーカーは文法要素 `TOKEN` (トークン ID) で書く** — `<\|tool_call>` / `<tool_call\|>` の綴りは通常トークンの列 (`<`, `\|`, `tool`, …) でも作れてしまい、リテラルで書くとその列も文法を通る。読む側 (`StructuredAssistantDecoder`) は tool call を**トークン ID** で切り出すので綴りの列は call にならず、描き直しもマーカートークンを書くので INV-1 の理由もそのまま当てはまる。実測では `tool_choice: required` でモデルが開きを綴り、閉じに本物の `<tool_call\|>` を使って 500 になった (SPEC §12 **DEV-22**)。 |
...
| CACHE-2 | **部分一致はそのまま使う。**不一致点から後ろだけを prefill する。all-or-nothing にしない。**memory を再利用点まで切り詰められないファミリ (再帰状態) では、CACHE-8 のチェックポイントが部分再利用の代替である** — そこで失うのは「直近のチェックポイント以降」であって会話全体ではない。参照実装も同じ形で、使えるチェックポイントが 1 つも無いときだけ 全再処理に落ちる (`server-context.cpp:3284` の `do_reset`、註が SWA / hybrid・再帰 memory を名指ししている)。**「1 トークン違えば全ミス」を実装の都合として持つことは、この行に反する。** |
...
| CACHE-8 | **コンテキストチェックポイント。**prefill の途中で推論状態のスナップショットを取る。間隔の下限は `--checkpoint-min-step`、1 スロットあたりの本数は `--ctx-checkpoints` (FLAG-8)。**最後の user メッセージの直前とプロンプト末尾付近では、間隔に関わらず取る** (参照実装 `server-context.cpp:3529`) — そこが次のターンで分岐が起きる位置だからである。再利用点まで memory を切り詰められないときは、**再利用点以前で最も新しいチェックポイントを restore してそこから prefill する** (`server-context.cpp:3272`)。使えるものが 1 つも無いときだけ全再処理 (CACHE-2)。 |
| CACHE-9 | **状態キャッシュ。**完了した状態を RAM 予算 (`--cache-ram`、FLAG-8) の中で複数保持し、新しい要求には**保持している状態のうち、新しいプロンプトとの lcp が最大でありかつ元の長さを最もよく残すもの**を restore してから CACHE-1 を判定する (参照実装 `server-task.cpp:1790` `server_prompt_cache::load` — `f_sim` と `f_keep` の 2 条件を同時に満たすものだけを選び、`f_keep < 0.25` は候補から外す)。生成スロットが 1 つでも (DEV-3)、会話が 2 本以上あるときに効く。 |
...
| FLAG-8 | CACHE-8 / CACHE-9 のフラグは参照実装と同名・同義: `--ctx-checkpoints` (`-ctxcp`)、`--checkpoint-min-step` (`-cms`、0 = 下限なし)、`--cache-ram` (`-cram`、-1 = 無制限 / 0 = 無効)。**既定値だけはこの機体で決める** (§12 DEV-23) — 参照実装の既定 (32 本 / 8192 トークン間隔 / 8192 MiB) は、1 本あたり再帰状態だけで 61.4 MiB 要るこの機体には載らない。 |
...
| DEV-14 | 投機デコードを落とす要求 | 強制挿入 (`rbudget`) もサンプラ chain の中にあるので、どの要求も投機と併用できる | **`repeat_penalty != 1` の要求だけが投機デコードを使わない** (plain 経路に落ちる)。**文法は GEN-14 (2026-08-21) で、思考の終了タグの強制 (RSN-4) は 2026-09-04 で、この列から外れた** — forcer は投機ループの中で動く: 強制する位置のトークンは引かずに採用し、ドラフトがたまたま同じトークンでない限りブロックをそこで切る。切った後ろは他のミスマッチと同じく捨てるので本文は plain と同じ。Mac アプリはツールを宣言する初回ラウンド全部に思考予算を付ける (WEB_SEARCH §6) ので、そこが毎ターン 450〜540 トークンを半分の速さで生成していた | 強制挿入は「引かずに置くトークン」、repetition penalty は「まだ確定していない履歴に依存する引き」であり、どちらもブロック内の後続位置が検証された前提を崩す。参照実装はどちらもサンプラとして chain に入れてあるので併用できるが、こちらの `ReasoningBudgetForcer` は復号ループの側にあり、サンプラの中にいない。**2026-08-21 に GEN-14 で文法だけを外した** — 文法は引き直し (GEN-7) を持ち込むが、引き直しても位置ごとに引き直すのは target 自身であり、食い違った位置から後ろは捨てるので前提は崩れない (参照実装 `common_sampler_sample_and_accept_n` がそう書いてある)。この 2 つを外に残したのは、実装の形が違うからであって理由が同じだからではない | 強制挿入をサンプラ側へ移したら (思考 ON の常用セッションに MTP が要ると分かった時点で読み直す)。repetition penalty はサンプラが履歴をブロック内で確定できるようになったら |
| DEV-15 | オブジェクトのプロパティ順 | 宣言順 (llama.cpp の json-schema-to-grammar は必須を先に宣言順、次に省略可を宣言順) | **Gemma: キーの昇順** (必須・省略可のどちらも)。**Qwen (Ornith / Qwen3.8) の tool call の引数: 必須を先、次に省略可、それぞれ宣言の原文 (`FunctionDefinition.parametersSource`) の順**、原文が無ければキーの昇順 (`QwenToolDeclaration.parameterOrder`、2026-09-15、docs/qwen38/24 §3)。値の中のオブジェクトはどちらも昇順 | (1) `JSONValue.object` は順序を持たない Swift の Dictionary である。(2) Gemma のテンプレートは tool_call の引数を `dictsort` で描くので、生成もキー昇順でなければ tool_call ターンで INV-1 (描き直し == 生成) が破れる。Qwen のテンプレートは `arguments|items` (呼び出しが持つ順) で描くので、昇順は上流の形ではなく、しかも**昇順が先に来る省略可の引数を閉ざしていた**: `fetch_page(url, sections?)` で `url` から書いたモデルは `sections` を足せず、同じページを取り直した。Qwen は解読器が書かれた順の JSON を返し (`ParsedToolCall.argumentsJSON`)、描き直しは `HistoricalToolCall.argumentsSource` の順 (無ければ文法の順) で書く (swift-jinja に順序つきの `Value.object` を渡す)。OpenAI 経路の宣言には原文が無いので、必須が先・昇順になる。**スキーマが記述していない自由形式の値の中のキーは、これでは閉じられない** — GBNF に文字列の比較が無いので文法で昇順を要求できず、`JSONValue.object` にも元の順序が残っていない。描き直しは `JSONValue.encoded()` の昇順で書き、モデルが別の順で書いたターンはそこで LCP が切れる (CACHE-8 により、失うのはそのターンの再 prefill だけ) | JSONValue が順序を持つようになり、かつ Gemma のテンプレートが宣言順で描くようになったら。OpenAI 経路が宣言の原文を持つようになったら Qwen の原文なしの場合を外す |
...
| DEV-23 | CACHE-8 / CACHE-9 の既定値 | チェックポイント 32 本 / 間隔 8192 / 状態キャッシュ 8192 MiB | **`--ctx-checkpoints` は 2** (`--checkpoint-min-step` と `--cache-ram` は未実装) | 再帰状態は文脈長に依存せず全 30 層で **61.4 MiB** 固定 (`RecurrentStateManager`)、これに full-attention 10 層ぶんの K/V が加わる。18GB 機で 32 本は載らない。本数・間隔・RAM 予算は実測で決める数字であり、参照実装の既定を写すべき数字ではない。2 は「このターンのプロンプト末尾と 1 つ前」= 123 MiB で、実測された分岐の形を覆う最小値 | 機体が変わったら / 測り直したら |
...
3. その行の**赤い適合テスト**を書く (テスト名に ID を含める)。
4. 実装して緑にする。
