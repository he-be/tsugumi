...
| DEV-15 | オブジェクトのプロパティ順 | 宣言順 | **キーの昇順**。必須・省略可のどちらも | (1) `JSONValue.object` は順序を持たない Swift の Dictionary である。(2) テンプレートが tool_call の引数を `dictsort` で描くので、生成もキー昇順でなければ tool_call ターンで INV-1 (描き直し == 生成) が破れる。**スキーマが記述していない自由形式の値の中のキーは、これでは閉じられない** — GBNF に文字列の比較が無いので文法で昇順を要求できず、`JSONValue.object` にも元の順序が残っていない。描き直しは `JSONValue.encoded()` の昇順で書き、モデルが別の順で書いたターンはそこで LCP が切れる (CACHE-8 により、失うのはそのターンの再 prefill だけ) | JSONValue が順序を持つようになり、かつテンプレートが宣言順で描くようになったら |
