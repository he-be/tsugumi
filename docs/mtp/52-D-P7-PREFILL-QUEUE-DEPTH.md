...
- ~~**`iogpu.wired_limit_mb` を既定 (8192) に戻して回していない。**~~ **回した (§9)。**
  既定 (32 スロット) は素の 8192 で通る。
...
## 9. **素の `wired_limit` (8192) で回した** — 既定は通る、ガードだけが古い

2026-08-20、ユーザーが `iogpu.wired_limit_mb` を **8192** (再起動時の既定) に戻した
状態で 8 run。**すべて n=1**、`math` / **128 tok** / temp 0 / `trusted-install`、
同一セッションで連続。**§4 の表と絶対値を並べてはいけない** — 生成長が 256 ではなく
128 なので 46 §5a のぶん遅く出る。ここで読むのは**通ったか / peak** だけである。

| モデル | 腕 | slots | tok/s | ttft | peak |
| --- | --- | ---: | ---: | ---: | ---: |
| `sym` | mmap | 32 | 30.47 | 1.633 | **1.29 GB** |
| `sym` | mmap | 48 | 32.59 | 1.157 | **1.31 GB** |
| `sym` | mmap | 64 | — | — | **ガードが拒否** (8.82 > 8.59) |
| `sym` | pread | 32 | 16.21 | 1.353 | 4.50 |
| `sym` | pread | 48 | 24.21 | 1.400 | 6.11 |
| `qat` (vision) | mmap | 32 | 28.55 | 1.777 | **1.25 GB** |
| `qat` | mmap | 48 | — | — | **ガードが拒否** (9.02 > 8.59) |
| `qat` | pread | 32 | 14.59 | 1.505 | 4.85 |

- **既定 (32 スロット) は素の 8192 で回る。**デモの模型で peak **1.25 GB**。
  コンテキストを伸ばしても **64K / 32 スロットは通る** (peak 2.54 GB)。
- **mmap の腕の peak はスロット数に対して平ら** — 32 で 1.29、48 で 1.31 GB。
  51 §2 の「私有スロット 3.22 GB が消える」がスロット数と無関係になったということである。
- **`ExpertCacheBudget` は腕を見ていない。**`numLayers × slots × expertStride` を
  請求し続けるので、**確保されないバイトで構成を弾く**。40 §3 の
  「48 スロットには `sudo sysctl iogpu.wired_limit_mb=14336` が要る」の正体はこれで、
  **物理メモリではなく式である。**
- **同じ式は 128K コンテキストでも噛む** — `resident 1.51 + vision 1.15 +
  experts 3.57 (幻) + kv 3.31 + scratch 0.29 = 9.83 > 8.59` で拒否。幻を抜くと 6.26 GB。
### 9a. **直したのは範囲のほうである** (ユーザー判断 2026-08-20)

上の 2 つは同じ 1 行から出ている。**直し方は 2 つあった** — ガードの算術を腕ごとに
直すか、**そもそも範囲外を受け付けないようにするか**。後者を採った。

| | |
| --- | --- |
| 受け付ける範囲 | **スロットは 8/16/24/32 (上限 = 運用点)**、**コンテキストは 128K まで**。`RuntimeConfiguration.allowedExpertCacheSlots` / `.maximumContextTokens` の 1 か所で、CLI・サーバー・デモ・アプリが共有する |
| 既定 | **32 スロット** (以前は 48。48 は範囲外になった) |
| ガードの算術 | **確保するバイトだけを数える** — mmap の腕では expert は 0 で、常駐要求は `summary` に併記するだけ。これで **128K / 32 スロットが通る** (実測 peak 3.88 GB) |
| 安全側は誰が持つか | **範囲である。**mmap の腕ではスロットを増やしても footprint が動かないので、**踏み込んでも気付けない** — 64 で tok/s が半分 (14.60 対 30.47、8K)、112 で常駐要求 11.3 GB となりスワップした (どちらも n=1、後者は強制終了)。**受け付けなければ起きない。** |

- **`pread` の腕は何も変わっていない** — 私有スロットは実際に確保するので今も
  合計に入り、128K / 32 スロットは 9.83 GB で**正しく弾かれる** (実測)。
- **未実施**: 常駐要求が working set をどれだけ押すかを**数として**測ってはいない。
  範囲を 32 で止めたので**要らなくなった問い**である (上限を上げたくなったら要る)。

## 10. このプローブが答えていないこと (§9 の分)

- **サーバー経路の A/B は依然として無い** (§8 のデモは体感のみ)。
- **§9 は n=1 である。**tok/s の差 (mmap 30.47 対 pread 16.21) を**測定値として引かない**
  — 40 §4-1 の規則どおり、ここは「通ったか」を見る表である。
