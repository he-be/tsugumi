...
public final class RemoteStreamingRepacker {
    ...
    private func runPrepared(paths: RemoteInstallPaths,
        ...
        if let visionPlan = plan.vision {
            try recordOutputFile(relativePath: MoEPackFormatV1.visionWeightsPath,
                                 path: visionPlan.resident.path,
                                 progress: progress)
        }
        if let draftPlan = plan.draft {
            try recordOutputFile(relativePath: MoEPackFormatV1.draftWeightsPath,
        ...
        let layoutData = try MoEPackJSON.encodeLayout(plan: plan, expertStride: expertStride)
        try writeSmall(path: layoutPath, data: layoutData)
        try MoEPackLayoutValidator.validate(path: layoutPath, plan: plan)
    ...
    private func writeManifest(plan: RepackPlan,
        ...
        var bits = MoEPackJSON.QuantBitWidths(
            embedding: 4,
            attention: 4,
            router: 8,
            sharedExpert: 8,
        ...
        for e in plan.resident.entries {
            ...
            if routerSuffixes.contains(where: { e.name.hasSuffix($0) }) {
                ...
                bits.router = e.quantSpec?.bits
                    ?? (e.dtype == MoEPackFormatV1.DType.bf16.rawValue ? 16 : bits.router)
            }
        ...
        let files = audit.outputFiles.map {
            ($0.relativePath, MoEPackJSON.FileEntry(size: $0.size, sha256: $0.sha256))
        }
        let data = try MoEPackJSON.encodeManifest(
            plan: plan,
