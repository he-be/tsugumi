...
public enum VerifiedInstallTool {
    ...
    public static let layoutMaxBytes: UInt64 = MoEPackFormatV1.packedExpertsLayoutMaxBytes

    public static func run(options: VerifyInstallOptions) throws -> VerifyInstallResult {
        let root = URL(fileURLWithPath: options.inputMoEPack).standardizedFileURL
        let access = try MoEPackDirectoryAccess(rootPath: root.path)
        ...
        let receiptData = try VerifiedInstallReceiptWriter.encode(
            ...
            toolVersion: "TsugumiRepack verify-install",
            files: files)
    ...
    private static func inspectFile(access: MoEPackDirectoryAccess,
    ...
    private static func loadManifest(data: Data) throws -> MoEPackManifestV1 {
        do {
            return try MoEPackManifestCodec.decode(data)
        } catch {
            throw RepackError.configurationInvalid(detail: "manifest.json invalid: \(error)")
        }
    }

    private static func loadLayout(data: Data) throws -> MoEPackPackedExpertsLayoutV1 {
        do {
            return try MoEPackPackedExpertsLayoutCodec.decode(data)
        } catch {
            throw RepackError.configurationInvalid(detail: "packed_experts/layout.json invalid: \(error)")
        }
    }

    private static func validatePackedExpertLayout(manifest: MoEPackManifestV1,
                                                   layout: MoEPackPackedExpertsLayoutV1) throws {
        do { try MoEPackV1StructuralValidator.crossValidate(manifest: manifest, layout: layout) }
        catch {
            throw RepackError.configurationInvalid(
    ...
    private static func findUnexpectedEntries(access: MoEPackDirectoryAccess,
                                              manifest: MoEPackManifestV1) throws -> [String] {
