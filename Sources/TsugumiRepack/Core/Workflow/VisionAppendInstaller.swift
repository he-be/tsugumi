...
///
/// `--include-vision` builds a whole `.moepack` from scratch, which for an
...
public final class VisionAppendInstaller {
    ...
    private func runPrepared(paths: RemoteInstallPaths,
        ...
        let weightsPath = (staging as NSString)
            .appendingPathComponent((MoEPackFormatV1.visionWeightsPath as NSString)
                .lastPathComponent)
        ...
        progress(.hashingOutput(MoEPackFormatV1.visionWeightsPath))
        ...
        try access.atomicWrite(try MoEPackManifestCodec.encode(updated), to: "manifest.json")
        ...
        audit.outputFiles.append(.init(relativePath: MoEPackFormatV1.visionWeightsPath,
                                       size: weightsSize,
                                       sha256: weightsSha))
    ...
    private func requireNoTower(_ manifest: MoEPackManifestV1) throws {
        guard manifest.vision == nil,
              manifest.flags["visionTower"] != true,
              manifest.files[MoEPackFormatV1.visionWeightsPath] == nil else {
            throw RepackError.configurationInvalid(detail: """
                \(options.inputMoEPack) already has a vision tower; reinstall the \
                model to change it
                """)
        }
    }
    ...
    static func manifestAddingVision(_ manifest: MoEPackManifestV1,
                                     plan: VisionFilePlan,
        ...
        guard files.updateValue(weights,
                                forKey: MoEPackFormatV1.visionWeightsPath) == nil else {
            throw RepackError.configurationInvalid(
                detail: "manifest already declares \(MoEPackFormatV1.visionWeightsPath)")
        }
        let config = plan.source.config
        return MoEPackManifestV1(
            magic: manifest.magic,
            versionMajor: manifest.versionMajor,
            versionMinor: max(manifest.versionMinor, MoEPackFormatV1.versionMinorVision),
            flags: flags,
            modelID: manifest.modelID,
            sourceSnapshotHash: manifest.sourceSnapshotHash,
            arch: manifest.arch,
            quant: manifest.quant,
            vision: MoEPackManifestVisionV1(
    ...
    private func verifyParity(vision: BoundVisionSource,
                              access: MoEPackDirectoryAccess) throws {
        let entries = try residentIndex(access: access)
        var byName: [String: MoEPackResidentIndexEntryV1] = [:]
    ...
    private func residentIndex(access: MoEPackDirectoryAccess) throws
        -> [MoEPackResidentIndexEntryV1] {
        let descriptor = try access.openFile(Self.textWeightsPath)
        defer { close(descriptor) }
        let path = "\(access.rootPath)/\(Self.textWeightsPath)"
        var headerBytes = [UInt8](repeating: 0, count: MoEPackFormatV1.residentHeaderBytes)
        ...
        let header = try headerBytes.withUnsafeBytes {
            try MoEPackResidentIndexCodec.decodeHeader($0)
        }
        guard header.indexSize <= MoEPackFormatV1.residentIndexMaxBytes,
              header.indexSize >= UInt64(MoEPackFormatV1.residentHeaderBytes) else {
            throw RepackError.configurationInvalid(
