...
enum ResidentWriter {
    ...
    static func encodeIndex(plan: ResidentFilePlan) throws -> Data {
        guard plan.indexSize <= UInt64(Int.max),
              plan.indexSize <= MoEPackFormatV1.residentIndexMaxBytes else {
        ...
        let (entryTableBytes, tableOverflow) = plan.entries.count
            .multipliedReportingOverflow(by: MoEPackBinary.indexEntryBytes)
        let (stringTableBase, baseOverflow) = MoEPackBinary.indexHeaderBytes
        ...
        for (index, entry) in plan.entries.enumerated() {
            guard entry.name.utf8.count <= Int(UInt16.max),
                  ...
                  MoEPackFormatV1.DType(rawValue: entry.dtype) != nil else {
                throw RepackError.configurationInvalid(
        ...
        MoEPackBinary.writeIndexHeader(into: idxBuf.baseAddress!,
        ...
        for i in 0..<plan.entries.count {
            let dst = idxBuf.baseAddress!.advanced(by: entriesBase + i * MoEPackBinary.indexEntryBytes)
            let nameOff = UInt32(stringTableBase) + plan.stringTableOffsets[i]
            MoEPackBinary.writeIndexEntry(into: dst, entry: plan.entries[i], nameOffset: nameOff)
        }
        ...
        do {
            try data.withUnsafeBytes { raw in
                ...
                _ = try MoEPackResidentIndexCodec.decodeRegion(raw, header: header)
            }
        } catch {
            throw RepackError.configurationInvalid(
