...
private let usage = """
Usage:
  TsugumiRepack --output <model.moepack> [--overwrite] [--resume]
  TsugumiRepack --output <model.moepack> --source-snapshot <dir> [--overwrite]
  TsugumiRepack --discard-partial --output <model.moepack>
  TsugumiRepack --add-vision --input-moepack <model.moepack>
  TsugumiRepack --add-draft --input-moepack <model.moepack>
  TsugumiRepack --verify-install --input-moepack <model.moepack>
  TsugumiRepack --help
    ...
    static func parse(_ values: [String]) throws -> Arguments {
        ...
        while index < values.count {
            ...
            case "--add-draft":
                ...
                index += 1
            case "--output", "--input-moepack", "--source-snapshot":
                guard index + 1 < values.count else {
                    throw ParseError.missingValue(flag)
                }
                switch flag {
                case "--output":     parsed.output = values[index + 1]
                case "--input-moepack": parsed.inputMoEPack = values[index + 1]
        ...
        if parsed.discardPartial {
            ...
            guard parsed.inputMoEPack == nil, parsed.sourceSnapshot == nil,
        ...
        if parsed.addVision {
            guard parsed.inputMoEPack != nil else {
                throw ParseError.missingRequired("--input-moepack")
            }
            guard parsed.output == nil, parsed.sourceSnapshot == nil,
                  !parsed.overwrite, !parsed.resume, !parsed.includeDraft else {
                throw ParseError.invalidMode("--add-vision accepts only --input-moepack")
            }
        ...
        if parsed.addDraft {
            ...
            guard parsed.output == nil, parsed.sourceSnapshot == nil,
                ...
                throw ParseError.invalidMode("--add-draft accepts only --input-moepack")
            }
        ...
        if parsed.verifyInstall {
            ...
            guard parsed.output == nil, parsed.sourceSnapshot == nil,
                ...
                throw ParseError.invalidMode("verification accepts only --input-moepack")
            }
        } else {
            guard parsed.output != nil else {
                throw ParseError.missingRequired("--output")
            ...
            guard parsed.inputMoEPack == nil else {
                throw ParseError.invalidMode("--input-moepack requires --verify-install")
            }
        }
        return parsed
    }
...
private func run(_ values: [String]) async -> Int32 {
    ...
    if arguments.addVision, let input = arguments.inputMoEPack {
        do {
            let result = try await VisionAppendInstaller(
                options: AddVisionOptions(
                    inputMoEPack: URL(fileURLWithPath: input).path,
    ...
    if arguments.addDraft, let input = arguments.inputMoEPack {
        do {
    ...
    if arguments.verifyInstall, let input = arguments.inputMoEPack {
        do {
            let result = try VerifiedInstallTool.run(
                options: VerifyInstallOptions(inputMoEPack: input))
