...
///
/// `--include-draft` builds a whole `.gturbo` from scratch, which for an
...
public final class DraftAppendInstaller {
    ...
    private func runPrepared(paths: RemoteInstallPaths,
        ...
        let weightsPath = (staging as NSString)
            .appendingPathComponent((GTurboFormatV1.draftWeightsPath as NSString)
                .lastPathComponent)
        ...
        progress(.hashingOutput(GTurboFormatV1.draftWeightsPath))
        ...
        let manifestData = try GTurboManifestCodec.encode(updatedManifest)
        ...
        let installedWeightsPath = (paths.finalDirectory as NSString)
            .appendingPathComponent(GTurboFormatV1.draftWeightsPath)
        ...
        audit.draftProvenanceTensors = draft.pin.provenanceTensors
            .map(\.repoName).sorted()
        audit.outputFiles.append(.init(relativePath: GTurboFormatV1.draftWeightsPath,
                                       size: weightsSize,
                                       sha256: weightsSha))
    ...
    private func requireNoDrafter(_ manifest: GTurboManifestV1) throws {
        guard manifest.draft == nil,
              manifest.flags["mtpDraft"] != true,
              manifest.files[GTurboFormatV1.draftWeightsPath] == nil else {
            throw RepackError.configurationInvalid(detail: """
                \(options.inputGTurbo) already has an MTP drafter; reinstall the \
                model to change it
                """)
        }
    }
    ...
    private func requireDraftFitsTarget(pin: DraftSourcePin,
                                        arch: GTurboManifestArchV1) throws {
    ...
    static func manifestAddingDraft(_ manifest: GTurboManifestV1,
                                    plan: DraftFilePlan,
        ...
        guard files.updateValue(weights,
                                forKey: GTurboFormatV1.draftWeightsPath) == nil else {
            throw RepackError.configurationInvalid(
                detail: "manifest already declares \(GTurboFormatV1.draftWeightsPath)")
        }
        return GTurboManifestV1(
            magic: manifest.magic,
            versionMajor: manifest.versionMajor,
            versionMinor: max(manifest.versionMinor, GTurboFormatV1.versionMinorDraft),
            flags: flags,
            ...
            draft: try GTurboJSON.draftSection(
