...
//   swift run -c release TsugumiKernelCheck --group-size 32
...
// `--vision-tower <model.moepack>` adds the assembled-tower comparison against
...
// `--draft <model.moepack>` adds the assembled-drafter comparison against the
...
// `--verify-block <model.moepack>` runs the M4 checks: the speculative verify
...
// `--qwen-open <model.moepack>` opens a repacked Qwen3.5-MoE install and reports
...
// `--qwen-tokenizer <model.moepack|tokenizer dir>` checks the Ornith tokenizer
...
// `--qwen-tools <model.moepack|tokenizer dir>` checks the XML tool call over the
...
// `--qwen-decode <model.moepack>` runs the decode path over the real weights and
...
//
// `--qwen-prefill <model.moepack>` is the same check with the prompt on the
...
// `--qwen-constrain <model.moepack>` drives the GEN-7 hook of
...
// `--qwen-resume <model.moepack>` splits one prompt across two requests and
...
// `--qwen-prefill-bench <model.moepack>` times the T-row path on a synthetic
...
if let index = arguments.firstIndex(of: "--qwen38-llkv-dump"), index + 2 < arguments.count {
    ...
    try runQwen38LLKVDump(
        tokenFile: arguments[index + 1], dir: arguments[index + 2], tokens: opt("--q38-tokens").flatMap { Int($0) } ?? 512,
        gguf: opt("--q38-gguf") ?? "~/LLM/Qwen3.8-Flash-Next-DS4-IQ2/Qwen3.8-Flash-Next-IQ2XXSImatrix-Q2KDownPad768-MTP.gguf",
        ple: opt("--q38-ple") ?? "~/LLM/Qwen3.8-Flash-Next-DS4-IQ2/ple/Qwen3.8-Flash-Next-PLE-Q4_1.gguf")
    exit(0)
}
...
if arguments.contains("--mmap-residency-probe") {
    var probeModel = "scratch/gemma4-qat-sym.moepack"
...
// Checks that need an installed `.moepack`. They share one context — and
