...
///
/// Prefers tokenizer sidecars in a completed `.moepack/tokenizer/` directory,
...
/// kernels consume.
///
/// Tsugumi owns the minimal chat framing because the upstream
...
public struct GFTokenizer: @unchecked Sendable {
    ...
    public static func tokenizerFolder(forModelDirectory modelDirectory: URL,
        ...
        guard let override = environment["TSUGUMI_TOKENIZER_DIR"], !override.isEmpty else {
            return nil
        }
    ...
    /// pinned and hash-validated, but `TSUGUMI_TOKENIZER_DIR` can point
