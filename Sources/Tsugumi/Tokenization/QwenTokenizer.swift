...
public struct QwenTokenizer: @unchecked Sendable {
    ...
    /// The tokenizer sidecar a repacked `.moepack` carries
    /// (`<model>/tokenizer/`), or `TSUGUMI_TOKENIZER_DIR`. There is no
    ...
    /// `TSUGUMI_TOKENIZER_DIR` can point anywhere; without this check a
