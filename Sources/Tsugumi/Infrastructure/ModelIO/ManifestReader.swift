...
public enum ManifestReader {
    ...
    public static let knownFlags: Set<String> = MoEPackFormatV1.knownFlags
    ...
    public static func load(directoryURL: URL,
        ...
        let directory = try MoEPackModelDirectory(rootURL: directoryURL)
        let data: Data
        do {
    ...
    package static func decode(data: Data,
        ...
        do {
            let wire = try MoEPackManifestCodec.decodeUnchecked(data)
            guard wire.magic == MoEPackFormatV1.magic
                    || wire.magic == MoEPackFormatV1.legacyMagic else {
                throw ModelError.notAMoEPackDirectory
            }
            guard wire.versionMajor == MoEPackFormatV1.versionMajor,
                  wire.versionMinor >= 0 else {
                throw ModelError.unsupportedVersion(major: wire.versionMajor,
                                                    minor: wire.versionMinor)
            }
            for key in wire.flags.keys where !MoEPackFormatV1.knownFlags.contains(key) {
                throw ModelError.unknownFlag(name: key)
            }
            if wire.expertStride % MoEPackFormatV1.alignmentBytes != 0 {
                throw ModelError.expertStrideNotPageAligned(
                    stride: wire.expertStride,
                    pageSize: Int(MoEPackFormatV1.alignmentBytes))
            }
            try MoEPackManifestCodec.validate(wire)
    ...
    private static func validateQuant(_ quant: ManifestQuant, isGemma4: Bool) throws {
        ...
        for (name, slot, allowedBits) in slots {
            ...
            if slot.weightBits == 16 {
                ...
                // compiled with `MOEPACK_AFFINE_SYMMETRIC` -- derives it
...
private extension ManifestQuantSlot {
    init(wire: MoEPackManifestQuantSlotV1) {
...
private extension ManifestQuant {
    init(wire: MoEPackManifestQuantV1) {
...
private extension ManifestVision {
    init(wire: MoEPackManifestVisionV1) {
        self.init(hiddenSize: wire.hiddenSize,
                  numLayers: wire.numLayers,
                  numHeads: wire.numHeads,
                  numKVHeads: wire.numKVHeads,
...
private extension ManifestDraft {
    init(wire: MoEPackManifestDraftV1) {
        self.init(hiddenSize: wire.hiddenSize,
                  numLayers: wire.numLayers,
                  numHeads: wire.numHeads,
                  numKVHeads: wire.numKVHeads,
