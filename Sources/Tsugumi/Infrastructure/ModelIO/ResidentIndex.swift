...
enum ResidentIndexReader {
    static let defaultMaxBytes = MoEPackFormatV1.residentIndexMaxBytes
    ...
    package static func load(fileDescriptor fd: Int32,
                             displayPath: String,
                             maxBytes: UInt64 = defaultMaxBytes) throws -> ResidentIndex {
        let headerBytes = MoEPackFormatV1.residentHeaderBytes
        ...
        let wireHeader: MoEPackResidentIndexHeaderV1
        do {
        ...
        let wireEntries: [MoEPackResidentIndexEntryV1]
        do {
            wireEntries = try indexBuf.withUnsafeBytes {
                try MoEPackResidentIndexCodec.decodeRegion($0, header: wireHeader)
            }
        } catch {
            throw ModelError.indexCorrupt(detail: "\(error)")
        }
