...
/// constant** injected into that source as `MOEPACK_AFFINE_GROUP_SIZE`. One
...
public final class MetalContext: @unchecked Sendable {
    ...
    ///
    /// `sym` compiles the library with `MOEPACK_AFFINE_SYMMETRIC = 1`, which
    ...
    /// `MOEPACK_AFFINE_GROUP_SIZE` is what every `k*GroupSize` constant in the
    ...
    private static func compileOptions(affineGroupSize: Int,
        ...
        opts.preprocessorMacros = [
            "MOEPACK_AFFINE_GROUP_SIZE": NSNumber(value: affineGroupSize),
            "MOEPACK_AFFINE_SYMMETRIC": NSNumber(value: affineScheme == .sym ? 1 : 0),
        ]
        return opts
    }
