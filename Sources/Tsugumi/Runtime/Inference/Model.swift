...
/// Loaded `.moepack/` model. Resident weights live behind one mmap'd
...
public struct Model {
    ...
    public func residentWeightBits(_ name: String) -> Int? {
        guard let entry = residentIndex.entries[name],
              entry.dtype == MoEPackFormatV1.DType.u32.rawValue,
    ...
    let modelDirectory: MoEPackModelDirectory
    ...
    init(device: MTLDevice,
         ...
         manifest: Manifest,
         directoryURL: URL,
         modelDirectory: MoEPackModelDirectory,
        ...
        self.streamersQueue = DispatchQueue(label: "tsugumi.expert-streamers")
        self.visionBox = VisionWeightsBox()
        self.visionQueue = DispatchQueue(label: "tsugumi.vision-weights")
        self.draftBox = DraftWeightsBox()
        self.draftQueue = DispatchQueue(label: "tsugumi.draft-weights")
    ...
    func resident(name: String) throws -> TensorView {
        ...
        // `MOEPACK_AFFINE_SYMMETRIC` -- derives `-8 * scale` without reading it.
...
extension Model {
    ...
    /// Open a `.moepack/` directory and return a typed handle. Eagerly verifies
    ...
    private static func validateTrustedReceiptLayerLayout(modelDirectory: MoEPackModelDirectory,
                                                          manifest: Manifest,
