...
package struct MoEPackResidentIndexHeaderV1: Equatable, Sendable {
...
package struct MoEPackResidentIndexEntryV1: Equatable, Sendable {
...
package enum MoEPackResidentIndexCodec {
    package static func decodeHeader(_ bytes: UnsafeRawBufferPointer) throws -> MoEPackResidentIndexHeaderV1 {
        guard bytes.count >= MoEPackFormatV1.residentHeaderBytes else {
            throw MoEPackFormatError.truncated(field: "resident.header")
        }
        let base = bytes.baseAddress!
        return MoEPackResidentIndexHeaderV1(
    ...
    package static func decodeRegion(_ bytes: UnsafeRawBufferPointer,
                                     header: MoEPackResidentIndexHeaderV1) throws -> [MoEPackResidentIndexEntryV1] {
        guard header.indexSize <= MoEPackFormatV1.residentIndexMaxBytes else {
        ...
        guard header.indexSize <= UInt64(bytes.count),
              header.indexSize >= UInt64(MoEPackFormatV1.residentHeaderBytes),
              header.indexSize % MoEPackFormatV1.alignmentBytes == 0 else {
            throw MoEPackFormatError.truncated(field: "resident.index")
        }
        let tableBytes = try moepackCheckedMultiply(header.entryCount,
                                                   UInt64(MoEPackFormatV1.residentEntryBytes),
                                                   field: "resident.entryTable")
        let tableEnd = try moepackCheckedAdd(UInt64(MoEPackFormatV1.residentHeaderBytes),
                                            tableBytes, field: "resident.entryTable")
        guard tableEnd <= header.indexSize, header.entryCount <= UInt64(Int.max) else {
            throw MoEPackFormatError.invalid(field: "resident.entryTable", reason: "outside index")
        }
        let residentEnd = try moepackCheckedAdd(header.indexSize, header.residentSize,
                                               field: "resident.payload")
        let base = bytes.baseAddress!
        var result: [MoEPackResidentIndexEntryV1] = []
        ...
        for index in 0..<Int(header.entryCount) {
            let offset = MoEPackFormatV1.residentHeaderBytes + index * MoEPackFormatV1.residentEntryBytes
            ...
            guard readU8(entry, 7) == 0 else {
                throw MoEPackFormatError.invalid(field: "resident.entries[\(index)].reserved",
                                                reason: "must be zero")
            }
            let nameEnd = try moepackCheckedAdd(nameOffset, nameLength,
                                               field: "resident.entries[\(index)].name")
            guard nameOffset >= tableEnd, nameEnd <= header.indexSize,
                  nameLength <= UInt64(Int.max) else {
                throw MoEPackFormatError.invalid(field: "resident.entries[\(index)].name",
            ...
            guard MoEPackFormatV1.DType(rawValue: dtype) != nil else {
                throw MoEPackFormatError.invalid(field: "resident.entries[\(index)].dtype",
            ...
            result.append(MoEPackResidentIndexEntryV1(
    ...
    private static func validatePrimaryPayloadRange(offset: UInt64, size: UInt64,
                                                    ...
                                                    field: String) throws {
        guard size > 0 else {
            throw MoEPackFormatError.invalid(field: field, reason: "primary payload is empty")
        }
    ...
    private static func validateOptionalPayloadRange(offset: UInt64, size: UInt64,
        ...
        if size == 0 {
            guard offset == 0 else {
                throw MoEPackFormatError.invalid(field: field, reason: "absent payload offset must be zero")
            }
            return
        }
        try validateContainedPayloadRange(offset: offset, size: size,
    ...
    private static func validateContainedPayloadRange(offset: UInt64, size: UInt64,
                                                      ...
                                                      residentEnd: UInt64,
                                                      field: String) throws {
        let end = try moepackCheckedAdd(offset, size, field: field)
        guard offset >= header.indexSize, end <= residentEnd else {
            throw MoEPackFormatError.invalid(field: field, reason: "range outside resident payload")
        }
    }
