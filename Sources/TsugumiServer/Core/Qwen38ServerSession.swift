...
/// | prompt cache | only an exact continuation of the live state; no checkpoints yet (G-0 / G-2), so a regenerate or an edited turn is recomputed whole |
...
public actor Qwen38ServerSession: ServerInferenceBackend {
    ...
    public func generate(
        ...
        // CACHE-1 / CACHE-5 over the live state only: no checkpoints are recorded, so a hit is always the engine's own
        // position and anything else starts over.
        var reused = 0
        switch promptCache.match(promptIDs, cachePrompt: request.cachePrompt) {
        case .hit(let cached) where cached == engine.position:
            reused = cached
        case .hit(let cached):
            ServerLog.promptCache("qwen38 hit at=\(cached) but state=\(engine.position), re-prefilling \(promptIDs.count)")
            engine.reset()
            promptCache.invalidate()
        case .miss(let divergedAt):
            if let divergedAt {
                ServerLog.promptCache("miss diverged_at=\(divergedAt) held=\(promptCache.tokens.count)")
            }
            engine.reset()
            promptCache.invalidate()
        }
