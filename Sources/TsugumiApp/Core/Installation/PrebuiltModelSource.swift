...
/// A finished `.moepack` install published as plain files on Hugging Face.
///
...
public struct PrebuiltModelSource: Equatable, Sendable {
    ...
    /// below were taken from, so they stay until the Hub repositories are
    /// renamed by hand; a Hub rename leaves a redirect, so this constant can
    /// follow later rather than ahead.
